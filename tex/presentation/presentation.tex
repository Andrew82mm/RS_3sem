...
\begin{document}
	...
	\begin{frame}{Обоснование выбора 128 компонент}
	\begin{table}[h]
		\centering
		\caption{Зависимость метрик от размерности эмбеддингов}
		\begin{tabular}{|c|c|c|c|c|}
			\hline
			\textbf{Компоненты} & \textbf{Дисперсия} & \textbf{Размер (MB)} & \textbf{Время SVD (с)} & \textbf{Сжатие} \\
			\hline
	...
	\end{table}
	\begin{itemize}
		\item 32 → 64: прирост +4.70\% при удвоении размера и времени
		\item 64 → 128: прирост +5.54\% при удвоении размера, время ×1.74
		\item 128 → 256: прирост +7.24\% при удвоении размера, время ×2.06
		\item 256 → 512: прирост +9.44\% при удвоении размера, время ×2.58
	\end{itemize}
